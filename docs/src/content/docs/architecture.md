...
- **Idle work yields to everything.** Deferred semantic publishes run only when the job queue is empty, under an idle-interrupt token the frontend flips before enqueuing *any* job — an edit, a read, or a notification preempts in-flight idle work at the next recompute boundary; the abandoned unit unwinds without committing a partial memo and is requeued. A typing burst therefore costs one settled semantic computation, not one per keystroke — latest edit wins.
