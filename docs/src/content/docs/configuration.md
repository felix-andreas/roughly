...
- `typing` — report `type-mismatch` diagnostics, including function-call argument mismatches. See the [Typing guide](/typing/guide). Default `false`. A single file can opt in or out with a top-level `# typing:` comment.
