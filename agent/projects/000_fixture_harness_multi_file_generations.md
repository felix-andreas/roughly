...
That module is not responsible for:
...
- None currently recorded for this project.
...
- Let one `Package` contain:
  ...
  - package scripts such as scripts or tests that can see package symbols but do not contribute to the package-level namespace
...
- In the later testing framework built on top of that parsing:
  ...
  - `#++++ any` means present but unchecked contents
  - an earlier expectation carries forward until replaced, deleted, or moved
- The testing framework may combine parsed fixture data with `workspace` document state.
- The workspace model must cover the current document use cases in `roughly/src/server.rs`.
...
The workspace model should describe three layers:
...
- package scripts do not contribute to package-global names
...
The first module should provide explicit operations around the settled model:

- workspace creation
- package registration
- package-root registration and lookup
- document add or replace with explicit package-document, package-script, or standalone-script kind
...
- distinguish package, package-script, and standalone documents
...
### 1. Finalize the document-management module boundary [done]
...
- Define how paths map to document buckets in the public API:
  ...
  - explicit package script insertion
...
### 2. Build the incremental document-management module [done]

- Introduce the new module.
- Reuse the same `Rope`, `Tree::edit`, and reparsing behavior currently used by `roughly`.
- Support:
  - add package
  - add or replace a document with explicit package-document, package-script, or standalone kind
  - edit document range
  - delete document
  - move document
...
### 3. Add direct tests for the document-management module [done]

- Place these tests next to the `workspace` module implementation.
- Cover:
  - package registration
  - package document add or replace
  - package script add or replace
...
- Cover the invariants explicitly:
  ...
  - package scripts do not become package documents by accident
...
- Define the exact grammar for:
  ...
  - expected-output attachment to the immediately preceding file or operation
  - `#++++ any` unchecked-output markers
...
### 5. Use the new fixture and workspace layers for typing fixture cases [planning]
...
### 6. Adopt the document-management module in `roughly` [planning]
...
- Make the `roughly` server use the shared document-management module for file state and parse updates.
