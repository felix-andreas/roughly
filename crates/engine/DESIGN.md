...
| `typecheck(f)` | `lower(f)`, `local_naming(f)`, `config`, and `global_scheme(s)` for **each cross-file or forward same-file symbol `s` that `f` references** | HM inference over the file. Records a dependency on exactly the interface symbols it reads — nothing more. A same-file reference whose defining assignment ends before its first read is *walk-shadowed*: the winner path rebinds the global entry at the definition site, so no import (and no interface edge) exists for it — which is what keeps a same-file reference chain from forming a fake SCC |
