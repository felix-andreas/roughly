...
fn node_ids() {
    ...
    let node_types = serde_json::from_str::<Vec<Value>>(tree_sitter_r::NODE_TYPES)
        .expect("node types should deserialize");
    let named: Vec<&str> = node_types
        .iter()
        .filter(|item| item["named"].as_bool().unwrap_or(false))
