...
use typing::{TypeChecker, TypeCheckError};
    ...
    match type_checker.extract_type_annotations(&source) {
        ...
        Err(type_error) => {
            ...
            let diagnostic = error(
                node,
                format!("Type annotation error: {}", type_error),
            );
            diagnostics.push(diagnostic);
        }
    }
            ...
            if let Some(assigned_value) = extract_assigned_value(line) {
                ...
                if let Err(type_error) = type_checker.check_assignment(&var_name, &inferred_type) {
                    return Some(create_type_error_diagnostic(
                        line_num,
                        line,
                        format!("Type mismatch in assignment to '{}': {}", var_name, type_error),
                    ));
                }
            }
        }
...
mod tests {
    ...
    fn test_extract_variable_name() {
        ...
        assert_eq!(extract_variable_name("my_var <- 'hello'"), Some("my_var".to_string()));
        assert_eq!(extract_variable_name("result = compute()"), Some("result".to_string()));
    ...
    fn test_extract_assigned_value() {
        ...
        assert_eq!(extract_assigned_value("name <- 'hello' #: character"), Some("'hello'"));
