...
//
// S4 goto-definition reference resolution
//

use {
    roughly::index::{s4_definition_ranges, s4_reference_at, S4Reference, S4ReferenceKind},
    tree_sitter::Point,
};

fn point_after(text: &str, marker: &str) -> Point {
    let end = text.find(marker).expect("marker present in fixture") + marker.len();
    let prefix = &text[..end];
...
fn reference_at(text: &str, marker: &str) -> Option<S4Reference> {
    let rope = Rope::from_str(text);
    let tree = tree::parse_rope(&mut tree::new_parser(), &rope, None);
    s4_reference_at(tree.root_node(), &rope, point_after(text, marker))
}

fn definition_count(text: &str, marker: &str) -> usize {
    let reference = reference_at(text, marker).expect("expected an S4 reference");
    let items = setup_flat(text);
    s4_definition_ranges(&reference, &items).len()
}

#[test]
fn s4_class_reference_in_setmethod_signature_resolves_to_setclass() {
    let text = indoc! {r#"
		setClass("Animal", representation(name = "character"))
		setMethod("speak", "Animal", function(x) x@name)
	"#};
    assert_eq!(
        reference_at(text, r#"setMethod("speak", "A"#),
        Some(S4Reference {
            name: "Animal".into(),
            kind: S4ReferenceKind::Class,
        })
    );
    assert_eq!(definition_count(text, r#"setMethod("speak", "A"#), 1);
}

#[test]
fn s4_generic_reference_in_setmethod_resolves_to_generic_and_methods() {
    let text = indoc! {r#"
		setGeneric("speak", function(x) standardGeneric("speak"))
		setMethod("speak", "Animal", function(x) x)
	"#};
    assert_eq!(
        reference_at(text, r#"setMethod("s"#),
        Some(S4Reference {
            name: "speak".into(),
            kind: S4ReferenceKind::GenericOrMethod,
        })
    );
    // resolves to the generic plus the method definition of the same name
    assert_eq!(definition_count(text, r#"setMethod("s"#), 2);
}

#[test]
fn s4_class_reference_in_new_resolves_to_setclass() {
    let text = indoc! {r#"
		setClass("Beast")
		make <- function() new("Beast")
	"#};
    assert_eq!(
        reference_at(text, r#"new("B"#),
        Some(S4Reference {
            name: "Beast".into(),
            kind: S4ReferenceKind::Class,
        })
    );
    assert_eq!(definition_count(text, r#"new("B"#), 1);
}

#[test]
fn s4_class_reference_in_vector_signature_resolves_to_setclass() {
    let text = indoc! {r#"
		setClass("Animal")
		setMethod("combine", c("Animal", "Other"), function(x, y) x)
	"#};
    assert_eq!(
        reference_at(text, r#"c("A"#),
        Some(S4Reference {
            name: "Animal".into(),
            kind: S4ReferenceKind::Class,
        })
    );
    assert_eq!(definition_count(text, r#"c("A"#), 1);
}

#[test]
fn plain_string_is_not_an_s4_reference() {
    let text = "greeting <- \"hello\"\n";
    assert_eq!(reference_at(text, "greeting <- \"h"), None);
}
