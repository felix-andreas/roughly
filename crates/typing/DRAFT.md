...
Use:
- `SEMANTICS.md` for the current user-facing semantics contract
- fixture expectations under `tests/fixtures/` for the user-visible behavior contract
- `ARCHITECTURE.md` for implementation architecture and internal design constraints

If anything in this file disagrees with `SEMANTICS.md` or the fixture suite, treat this file as outdated draft material.

## Purpose

Keep this document only for:

- earlier semantics sketches that may still be useful as discussion starting points
- ideas that were intentionally deferred
- future-facing design directions that have not been agreed on yet

Do not implement behavior from this file without discussing it with the user first and then recording the agreed result in `SEMANTICS.md`.
