...
- `roughly` maintains real document state in one long-lived `typing::Analysis`
- `typing` runs against real document paths instead of the temporary `current.R` adapter
...
- migrate document outline/symbol indexing into `typing`
...
- eliminate duplicated parse state between `roughly` and `typing`
...
- retain package context in `typing::Analysis` rather than dropping closed documents eagerly
- preload package files from `R/` into `typing::Analysis` during server initialization
- when watched non-open package files change, mark them dirty and refresh them before the next full
  typing run
- use real workspace-relative document paths in `typing::Analysis`
...
- `initialize`
  ...
  - preload package files from `R/` into `typing::Analysis`
- `did_open`
  - update both the existing open-document parse state and the corresponding `typing::Analysis`
    document
  - keep publishing the current non-typing diagnostics path
- `did_change`
  - update only the existing fast `roughly` diagnostics path
  - update the open-document copy in `typing::Analysis`
  - do not run typing diagnostics yet
- `did_save`
  - refresh any dirty non-open package files in `typing::Analysis`
...
- `did_close`
  ...
  - retain the file in `typing::Analysis` as package context
...
For this milestone, `typing` does not need a new fast frontend diagnostics API.
...
1. `typing` package ordering is still insertion-order rather than package collation order.
2. `roughly` does not yet preload package files into `typing::Analysis`.
...
- duplicated parse state between `roughly` and `typing`
...
### 2. Make package ordering explicit in `typing` [pending]
...
- parse package files from disk into `typing::Analysis` using their real paths
...
- add server-side helpers that:
  - insert or replace one `typing::Analysis` document from open-buffer contents
...
- before the next full typing run:
  - reload dirty files from disk into `typing::Analysis`
  - delete removed files from `typing::Analysis`
...
- it keeps `typing::Analysis` coherent enough for package-wide save diagnostics

### 6. Wire milestone behavior into `did_open` / `did_change` / `did_save` / `did_close` [pending]

- `did_open`
  - update `typing::Analysis` from the opened buffer contents
...
- `did_change`
  ...
  - mirror the text edits into `typing::Analysis`
- `did_save`
  - synchronize dirty non-open files into `typing::Analysis`
  - run full typing analysis
  - publish typing diagnostics for the saved file
- `did_close`
  - remove only the open-buffer parse state from `roughly`
  - keep package context warm in `typing::Analysis`
...
- package files from `R/` are present in `typing::Analysis` even when not open
...
- decide whether `roughly` or `typing` should own the shared parse state long-term
- add tree-sitter node ids to HIR when typing-backed hover/goto-definition starts
- migrate outline/document symbols into `typing` if that still looks worthwhile
