...
- **`.agents/memory/MEMORY.md`** — the agent knowledge base. Three horizons (short / mid / long-term; see its header). Engineering state, active priorities, technical debt, and non-obvious design rationale live here so they are not rediscovered. Update it in the same session as the work; prune the short-term section aggressively and delete weak or duplicated context.
