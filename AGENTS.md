...
- **`.agents/memory/` — the agent knowledge base, kept IN THE REPOSITORY.** `MEMORY.md` is the **index**: it holds durable engineering state and non-obvious design rationale inline, and is the single place that names every other knowledge document. Keep a *separate* document only for genuinely larger-scope material — currently `backlog.md` (the prioritized work punch-list) and `decisions.md` (the architecture decision log) — and reference it from `MEMORY.md`; never spawn a new knowledge file for something small (inline it into `MEMORY.md` instead). Keeping this current — **adding** what is durable **and pruning** what is resolved, stale, or duplicated — is part of repository hygiene, not optional; do it in the same session as the work.
