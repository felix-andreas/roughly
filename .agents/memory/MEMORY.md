...
- **The language is being renamed to `ry`** (user decision): lowercase `ry` in prose and identifiers, `Ry/` for the dialect source directory beside `R/`, `ry.toml`, `# ry: allow(...)`, `RY_*` variables, `.ry` sources and `base.ry.stub` declarations, with documentation at ry-lang.org. `backlog.md` §"rename the language and toolchain to `ry`" sequences the work by blast radius and marks the three surfaces that are the user's call rather than an agent's (crates.io name, Marketplace extension id, repository name).
