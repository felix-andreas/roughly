...
- The edited file can be inferred more than once per demand path (`Typecheck(f)` and `ExportedSchemes(f)` each run `infer_file`; genuinely cyclic groups re-infer per fixed-point round — same-file backward chains no longer do, decisions.md). A keystroke in a huge file still pays one whole-file authoritative re-inference; going below that needs partial/deferred re-inference — a design, not a tweak. Revisit a shared inference only with a design that doesn't retain every file's `ModuleCheck` for exports.
