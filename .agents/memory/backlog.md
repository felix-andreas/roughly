...
- **CI exists but is mis-scoped — the core crate is ungated** *(CTO)*. `.github/workflows/ci.yml` runs `cargo fmt --all --check`, `clippy --all-targets --all-features -- -D warnings`, `build`, `test`, doctests + a second job for the `#[ignore]` benches. **But** the workspace has `default-members = ["crates/roughly"]` and the CI commands pass no `--workspace`, so the gate only covers `crates/roughly`: `analysis` (type checker + the whole fixture suite AGENTS.md calls the primary test command) and `engine` are compiled as dependencies but their **tests and lints never run in CI**. That is why CI is green with clippy warnings live in `analysis` and why no R is needed (rofy/zed aren't default-members). Fix: widen the gate to the whole workspace **excluding** `rofy` (needs R — experimental, see P2) and `editors/zed` (separate toolchain) — e.g. `--workspace --exclude rofy --exclude zed` — then clear the warnings it surfaces (see P1 clippy). Verify `cargo test -p analysis -p roughly` locally (rofy can't build without R here).
...
- **Clippy warnings in the ungated crates** *(CTO)* — surfaced once CI covers the workspace (P0 above); on stable, `cargo clippy -p analysis --all-targets` shows unused imports (`BTreeMap,BTreeSet,Diagnostic,DocumentId,Symbol` in the `analysis.rs` test module), collapsible `if`, manual `str::repeat`, unnecessary same-type cast, too-many-args (8/7); plus the earlier audit's `clone`-on-`Copy`, missing `Default`s, `contains_key`+`insert`, `expect_fun_call`. Clear them so the widened `-D warnings` gate is green. (`cargo fmt --all --check` already passes on stable — the `rustfmt.toml` `imports_granularity`/`group_imports = One` rules are nightly-only and silently skipped on the stable toolchain CI uses.)
