...
- **Stubs — proper format + standard-library coverage** *(CTO + Expert own the format; user hard requirement)*. Replace the `crates/analysis/src/stdlib_base.R` blob (≈12 base fns) with a proper, documented stub format. **DECIDED (user):** (a) **dedicated declaration-only stub files** — the Expert's `.Rstub` design (flat `name : type` declarations reusing the existing type-expression parser; a body is unrepresentable, enforcing declaration-only) is accepted **EXCEPT the extension name** — pick a different one (candidates: `.Rti` / `.Ri` / `.Rtypes` / `.Rdecl`; avoid `.Rd` = R docs, `.Rmd`, `.Rda`; CTO + Expert choose in the design pass). (b) **Overloads: use `Any` for v1** — but genuinely parametric higher-order functions (e.g. `lapply`, `Map`, `Reduce`, `identity`) must still get real generic types via the existing `<T> fn(...)` polymorphism, NOT `Any`; only ad-hoc overloads where the return type varies by argument type (e.g. `abs`, `rep`) fall back to `Any`/`Incomplete`. Whether those are ultimately solved by **traits** in the type system or by **overload sets** is deferred (revisit post-v1). Grammar must allow repeated declarations now so the corpus need not be rewritten later. Requirements still stand: (1) the chosen declaration format; (2) **syntax highlighting** — LSP semantic tokens first (one server impl covers inline `#:` + the stub files in both editors), tree-sitter/TextMate second; (3) real stdlib stubs (base, stats, utils, methods, …); (4) **overridable** (project stub wins over shipped). Rewrites the `stdlib.rs` harvester (drop the `function(x) 0L` placeholder path) and `stdlib-stubs.md` (drop "(Proposal)"). Full Expert proposal + open questions captured in `decisions.md`. "No new features, but stdlib stubs must exist."
