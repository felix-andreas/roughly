...
- The edited file can be inferred more than once per demand path (`Typecheck(f)` and `ExportedSchemes(f)` each run `infer_file`, and a file whose top-level symbols form interface SCCs re-infers per fixed-point round). The typing probe shows the push-server keystroke path runs 1 authoritative inference + 1 SCC body; the self-recursive fixed point is deliberate (tolerant `Unknown`, decisions.md), so those rounds stay. Revisit a shared inference only with a design that doesn't retain every file's `ModuleCheck` for exports.
