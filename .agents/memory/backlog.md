...
- **Letrec typecheck half:** a local recursive closure resolves its own name (naming DONE) but the recursive read types through the silent unbound-local `Unknown`. Pre-bind the target slot with a fresh inference variable for function-valued assignments and unify with the inferred function type (monomorphic recursion). Then part (b): top-level recursion converges to a scheme containing `Unknown` with no strict origin anywhere — the interface fixed point should record an origin on the binding whose converged scheme retains `Unknown`. Decide the mutual-local-recursion policy while there.
- **NSE follow-ups:** honor `utils::globalVariables(c(...))` declarations; generalize the data-mask into a `.Rtypes` parameter marker so dplyr verbs can declare masked positions (today: data.table bracket signature + with/within/subset/transform are recognized structurally).
- **Design forks, decide-and-implement (one `decisions.md` entry each):** third constraint kind (two-flexible-operand comparison — the recorded traits tripwire); order-dependent compatibility commits; NAMESPACE bare-resolution gating (user fork: should un-imported names resolve?).
