...
**Phase 3 (gate d) — wire `server.rs` + `cli.rs` to the engine, in green sub-increments (keep `test_lsp.rs` green throughout):**
...
**Pre-final-sign-off breadth residuals (Expert-flagged after accepting gate (b); do after Phase 3/4, none blocks the server rewire):**
1. **Extend the Class-1 granularity exec-counter proof to ALL per-keystroke features** — completion, inlay hints, signature help, local goto — not just hover. Completion especially fetches a different set (`PackageSymbolIndex` on top of `Typecheck`), so prove a point query on an unchanged file ⇒ 0 `Typecheck` reruns for each (`test_ide_granularity`).
2. **Add a randomized IDE differential stream** — reuse the diagnostic harness's `SplitMix64` generator + `sync_engine`, calling `sweep_parity` after every step. The curated IDE scenarios miss the long-tail invalidation bugs the 8-seed × 300-step diagnostic stream catches; mirror it for IDE.
3. **Close the strict-origin non-empty case** (gate (a) thinness surfaced by the Phase 2 recording fix): the diagnostic differential never forces a NON-EMPTY `strict_origins`, so the strict-origin rendering sub-path is unverified vs production. Add a curated strict case (an `Unsupported` construct or an O2 undetermined reference) that forces a non-empty origin and asserts parity.
